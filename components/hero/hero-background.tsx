...
interface HeroBackgroundProps {
  ...
  onTrailerEnded(): void;
  /** Callback function when video player is closed */
  onCloseVideo?: () => void;
...
export function HeroBackground({
  ...
  const { getEmbedUrl, selectedEpisode, seasonNumber } = useEpisodeStore();
  const { selectedServer, vidnestContentType, animePreference, audioLanguage } =
    useServerStore();
  ...
  const getVideoSrc = () => {
    ...
    if (detectedMediaType === "tv") {
      ...
      if (episodeEmbedUrl) {
        return applyAudioLanguageToUrl(
          episodeEmbedUrl,
          selectedServer.id,
          audioLanguage,
        );
    ...
    const movieUrl = selectedServer.getMovieUrl(media.id);
    return applyAudioLanguageToUrl(movieUrl, selectedServer.id, audioLanguage);
  };
        ...
        >
          ...
          {isPlayingVideo && (
            <StreamPlayer
              ...
              title={(media.title || media.name || "Media") as string}
