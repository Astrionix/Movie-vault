...
export function ContentCard({
  ...
  return (
    <Card
      className="group relative overflow-hidden rounded-xl bg-slate-900/60 backdrop-blur-md border border-white/10 hover:border-transparent hover:ring-2 hover:ring-primary/60 transition-all duration-300 ease-out shadow-lg hover:shadow-[0_16px_36px_rgba(0,0,0,0.85)] hover:scale-[1.08] hover:z-30 cursor-pointer h-full flex flex-col md:aspect-[2/3] select-none"
      onClick={navigate}
      onMouseEnter={prefetch}
    ...
    >
      ...
      <div className="relative flex-shrink-0 aspect-[2/3] overflow-hidden rounded-t-xl">
        <Poster
          posterPath={item.poster_path ?? undefined}
          title={title || "Poster"}
          size="medium"
          className="rounded-none transition-transform duration-500 ease-out group-hover:scale-110"
        />
        {/* Netflix Glowing Play Icon */}
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none z-20">
          <div className="w-12 h-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-2xl shadow-primary/60 transform scale-75 group-hover:scale-100 transition-transform duration-300 border border-white/30">
            <Icons.play className="w-5 h-5 ml-0.5 fill-current" />
          </div>
        </div>
      </div>

      <div className="p-3 relative flex-grow flex flex-col items-center text-center transition-all duration-300 md:absolute md:bottom-0 md:left-0 md:right-0 md:translate-y-2 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 z-30 md:bg-gradient-to-t md:from-black/98 md:via-black/85 md:to-transparent md:backdrop-blur-sm">
        <MediaLogo
          logo={item.logo}
          align="center"
          className="w-full max-h-9 mx-auto mb-1.5"
          title={title}
          fallbackClassName="text-sm font-bold tracking-tight leading-tight line-clamp-1 mb-1 text-white drop-shadow-md"
        />

        <div className="flex flex-col items-center gap-1.5 w-full mt-auto">
          <div className="flex flex-wrap justify-center items-center gap-2 text-[10px] font-semibold text-gray-300">
            {year && <span className="text-gray-300">{year}</span>}

            {item.vote_average && item.vote_average > 0 && (
              <>
                <span className="opacity-40">•</span>
                <div className="flex items-center gap-1 text-amber-400 bg-amber-400/10 px-1.5 py-0.5 rounded border border-amber-400/20">
            ...
            <span className="px-1.5 py-0.2 bg-white/10 border border-white/15 rounded text-[9px] font-bold text-gray-200">
              HD
            </span>

            {item.content_rating && (
              <span className="px-1.5 py-0.2 bg-white/10 border border-white/15 rounded text-[8px] font-bold text-gray-300 uppercase">
                {item.content_rating}
              </span>
            )}
          </div>
