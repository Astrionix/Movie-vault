...
export async function generateMediaMetadata({
  ...
  if (!media) {
    return {
      title:
        notFoundTitle ||
        `${mediaType === "movie" ? "Movie" : "TV Show"} Not Found | Movie Vault`,
      description:
        notFoundDescription ||
  ...
  const description = media.overview || `Watch ${title} on Movie Vault`;
  ...
  return {
    ...
    ...(mediaType === "movie" && {
      alternates: {
        canonical: `/movies/${mediaId}`,
        languages: {
          "en-US": `/movies/${mediaId}`,
        },
      },
    }),
    description,
